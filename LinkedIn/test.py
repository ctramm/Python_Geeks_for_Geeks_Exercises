...
def fuzzy_math(num1, operator, num2):
    if type(num1) != int or type(num2) != int:
        raise Exception("We need to do fuzzy math on its")
...
class TestFuzzyMath:
    ...
    def test_non_int_input_for_num1(self):
        with pytest.raises(Exception) as exc_info:
            fuzzy_math("hi", "+", 2)
        assert "We need to do fuzzy math on its" in str(exc_info)
    ...
    def test_multiplication_with_negative_result(self):
        assert "negative number" in fuzzy_math(-1, '*', 2)

    def test_multiplication_with_small_result(self):
        assert "small number" in fuzzy_math(1, '*', 2)

    def test_multiplication_with_medium_result(self):
        assert "medium-sized" in fuzzy_math(2, '*', 7)

    def test_multiplication_with_large_result(self):
        assert "really big" in fuzzy_math(11, '*', 2)

    def test_invalid_operator(self):
        with pytest.raises(Exception) as exc_info:
            fuzzy_math(1, '&', 2)
        assert "I don't know how to do math with" in str(exc_info)
